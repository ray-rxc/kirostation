...
### 주요 변경
- ACP 프롬프트 큐잉: 에이전트 실행 중 메시지를 큐에 저장, 턴 종료 시 자동 전송
...
- 코드 블록 높이 잘림 수정 (fixedSize + 안정적 Block id)
- 타이틀바 더블클릭 zoom 복원 (hiddenTitleBar 대응)
...
### 주요 변경
- 사이드바 새로고침 버튼 클릭 시 앱 업데이트 체크 추가
- 사이드바 대시보드 버튼 패딩 조정 (상하좌우 12pt)
- 에디터 접힌 상태에서 대시보드/파일 열기 시 자동 펼침
- 사이드바 섹션 펼침/접힘 상태 유지 + View 메뉴 토글

---

## v1.0.0-75 (2026-04-23)

### 주요 변경
- ACP 툴바: worktree/브랜치 분리 표시 (worktree 있는 repo면 폴더명 파란 캡슐 + 브랜치 보라 캡슐)
- ACP 채팅: ESC 키로 처리 중 취소 기능 + "esc to cancel" 안내 문구
- 세션 resume 시 에이전트 플래그 누락 버그 수정 (터미널 모드에서 --resume-id가 --agent 덮어쓰던 문제)
- worktree picker: 폴더명 우선 표시, main ★ 표시
- 대시보드 드래그→터미널 지원, 터미널 첫 클릭 수정
...
### 주요 변경
- QuickLauncher: 우클릭 컨텍스트 메뉴에서 에이전트 퀵 런처로 실행
  - 기존 깊은 메뉴(도구→프로젝트→에이전트) 제거, 1뎁스 도구 선택 → QuickLauncher 뷰
  - 프로젝트/Worktree/ACP모드/에이전트를 한 화면에서 선택
  - context 경로에서 프로젝트 자동 유추 (스펙 파일, 프로젝트 하위 경로)
  - 프리셋(즐겨찾기 조합) 즉시 실행 지원
  - tabs/grid 모드 모두 지원
  - 미시작 탭 1개만 있으면 재사용
...
- 프로젝트 카드에 IDE 자동 감지 및 열기 메뉴 (Xcode/Android Studio/VS Code/Finder)
- 프로젝트 카드 ⋯ 메뉴 추가 (수정/삭제)
- 앱 시작 시 외부 kiro-cli 세션이 종료되는 문제 수정
...
### 수정
- 에디터 포커스 시 CMD+C/V가 터미널로 전달되는 문제 수정
